...
layui.use('table', function(){
  ...
  table.render({
    elem: '#demo'
    ,height: 'full-230'
    ,url: con.app+'/rel/getList.do' //数据接口
    ,request:{
        pageName:'pageIndex'  //参数默认名字为page
        ,limitName:'pageLimit' //参数默认名字为limit
        }
    ,where :{roleName:$("input[name='roleName']").val(),menuName:$("input[name='menuName']").val()}
    ,page: true //开启分页
    ,cols: [[ //表头
      ...
      ,{field: 'menuCode', title: '菜单编号', width:220} 
      ,{field: 'menuName', title: '菜单名称', width:220} 
...
function openAdd(){
	openLayer("/rel/add.do",refresh);
}
function openUpd(id){
	openLayer("/rel/add.do?id="+id,refresh);
}
function del(id){
	openConfirm(function(index){
		ajax('/rel/del.do', {id:id}, 'text', function(data){
			console.log(data)
	        if (data == 1) { 
	            layer.msg('删除成功');
