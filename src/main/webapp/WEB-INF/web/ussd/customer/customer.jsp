...
<div class="layui-collapse">
  <div class="layui-colla-item">
    <h2 class="layui-colla-title">客户信息-查询条件</h2>
	...
	<fieldset class="layui-elem-field layui-field-title" style="margin-top: 0px; padding: 5px">
	  <form  method="post"   enctype="multipart/form-data">
			<div class="layui-form-item">
				...
				<input type="button" class="layui-btn"  id="test1" value="上传文件"  />
				</span>
			</div>
		</form>
	</fieldset>
...
layui.use('table', function(){
  ...
  table.render({
    elem: '#demo'
    ,height: 'full-230'
    ,url: con.app+'/customer/getList.do' //数据接口
    ,request:{
    ...
    ,cols: [[ //表头
      ...
      ,{field: 'status', title: '客户身份', width:180}
...
upload.render({
	elem: '#test1',
	url:"/RBAC1/customer/uploadExcel.do",
	acceept:'file',
	exts:"xlsx|xls",
	done:function(res){
		layer.msg("上传成功");
...
function exportExcel(){
	location.href="/RBAC1/customer/exportExcel.do";
}
function openAdd(){
	openLayer("/customer/add.do",refresh);
	
}
function openUpd(code){
	openLayer("/customer/add.do?code="+code,refresh);
}
function openEmail(email){
	openLayer("/customer/toEmail.do?email="+email,refresh);
}
function del(code){
	openConfirm(function(index){
		ajax('/customer/del.do', {code:code}, 'text', function(data){
			console.log(data)
	        if (data == 1) { 
	            layer.msg('删除成功');
...
<script id="barDemo" type="text/html">
 ...
 <input type='button' class='layui-btn layui-btn-danger' value='删除' 
           onclick='del("{{ d.code }}")'/>
	<input type='button' value='发送邮件' class='layui-btn' 
           onclick='openEmail("{{ d.email }}")'/>&nbsp;
</script>
</body>
</html>