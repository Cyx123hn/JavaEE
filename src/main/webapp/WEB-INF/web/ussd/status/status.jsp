...
<div class="layui-collapse">
  <div class="layui-colla-item">
    <h2 class="layui-colla-title">状态信息-查询条件</h2>
	  ...
	  <form  method="post"   enctype="multipart/form-data">
			<div class="layui-form-item">
				...
				<div class="layui-input-inline">
					<input type="text" name="code"  placeholder="请输入状态编号" autocomplete="off" class="layui-input">
				</div>
				<label class="layui-form-label">状态名称</label>
				<div class="layui-input-inline">
					<input type="text" name="name" placeholder="请输入状态名称" autocomplete="off" class="layui-input">
					</div>
				<span> 
...
layui.use('table', function(){
  ...
  table.render({
    elem: '#demo'
    ,height: 'full-230'
    ,url: con.app+'/status/getList.do' //数据接口
    ,request:{
...
function openAdd(){
	openLayer("/status/update.do",refresh);
}
function openUpd(code){
	openLayer("/status/update.do?code="+code,refresh);
}
function del(code){
	openConfirm(function(index){
		ajax('/status/del.do', {code:code}, 'text', function(data){
			console.log(data)
	        if (data == 1) { 
	            layer.msg('删除成功');
