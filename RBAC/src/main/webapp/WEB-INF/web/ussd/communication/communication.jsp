...
<div class="layui-collapse">
  <div class="layui-colla-item">
    <h2 class="layui-colla-title">沟通信息-查询条件</h2>
	  ...
	  <form  method="post"   enctype="multipart/form-data">
			<div class="layui-form-item">
				...
				<div class="layui-input-inline">
					<input type="text" name="custName"  placeholder="请输入客户姓名" autocomplete="off" class="layui-input">
				</div>
...
layui.use('table', function(){
  ...
  table.render({
    elem: '#demo'
    ,height: 'full-230'
    ,url: con.app+'/communication/getList.do' //数据接口
    ,request:{
        pageName:'pageIndex'  //参数默认名字为page
        ,limitName:'pageLimit' //参数默认名字为limit
        }
    ,where :{custName:$("input[name='custName']").val(),userName:$("input[name='userName']").val()}
    ,page: true //开启分页
    ,cols: [[ //表头
       {title:'全选',type:'checkbox',fixed:'left'}
      ,{title:"序号",type:"numbers",fixed:'left'}
      ,{field: 'id', title: 'ID', width:80, sort: true, fixed: 'left'}
      ,{field: 'custName', title: '客户姓名', width:80, sort: true}
...
function openAdd(){
	openLayer("/communication/add.do",refresh);
}
function openUpd(id){
	openLayer("/communication/add.do?id="+id,refresh);
}
function del(id){
	openConfirm(function(index){
		ajax('/communication/del.do', {id:id}, 'text', function(data){
			console.log(data)
	        if (data == 1) { 
	            layer.msg('删除成功');
